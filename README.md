...
また今回の開発ではフロントエンドとバックエンドを両方手がけました。  
フロントエンドのリポジトリは[こちら](https://github.com/yunu0705/My-React-App)からご確認ください。  
 ...
 * ログイン・ログアウト
 * スケジュール投稿・編集・削除  
・サイトに反映するスケジュールはその週の月曜日～日曜日までです。毎週日曜日0時に自動で次週の予定が切り替わるようになっています。（登録されていれば）  
 * ニュース投稿・編集・削除
・画像添付機能は任意であり、画像がなかったときはデフォルトでNO IMAGE画像が使われる設定になっています。
...
#### 詳細は [Railsを用いたバックエンドの機能と課題へのアプローチ](#railsを用いたバックエンドの機能と課題)に記載しています。  
...
# Railsを用いたバックエンドの機能と課題

### 機能  

### 1. 自動メール送信機能  

実装した自動メール送信機能では、ユーザー登録や問い合わせに対して、確認メールをリアルタイムに送信することを目指しました。この機能によりユーザーがスムーズな体験を得られるようにし、特にActiveMailerとSMTPサーバーを使った安全な送信方法を採用したいと思っています。  
...
## 課題へのアプローチ　　
(1) SMTP設定やCORS対応において本番環境でメール送信が失敗することがありました。SMTPホスト名やポート設定を詳細に検証しましたが、特定の環境での設定の相違が原因であることが判明。  
...
(2) 自動更新のタイミング設定がサーバーによって異なることがわかり、予定の反映が一日ずれるケースが発生。これに対し、グローバル時計設定の調整によって正確な予定登録が可能になりました。  

(3) 画像のアップロード時に、正しくデータベースに反映されないケースが発生。画像が反映されない原因を調査し、ActiveStorageの設定とファイルパスを細かく確認。さらに、今後の対応としては、S3などの外部ストレージとの連携も視野に入れ、安定した画像の取り扱いを目指しています。

(4) API経由でデータを送受信する際に、CORSエラーが発生。これに対し、Railsのcors.rbファイルで設定を調整し、CORSリクエストの許可範囲を制限することで安全性を確保しようとするもうまくいかずコードの見直しをしています。 
