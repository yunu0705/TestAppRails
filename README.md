...
## 📌 １分で読めるプロジェクト概要・背景  
本プロジェクトは、個人ストリーマー向けに、ファン向けの情報発信が簡単にできるオリジナルサイトのパッケージの開発・検証することを目的としています。発案者である知人から、「個人ストリーマーが手軽にスケジュールやグッズなどのニュースを積極的に発信できる専用サイトを提供できるのか」という相談を受けました。知人自身はかつて個人で配信活動を行っており、登録者数の伸び悩みやモチベーション維持の難しさに直面しました。配信活動を引退した彼は、同じ悩みを抱える個人ストリーマーの支援を目的としたサービスができないかと考え、このプロジェクトがスタートしました。私自身は開発経験を積むため、自己研鑽を目的として開発に参加しています。　
...
> # 📢 テスト時のご案内  
> 当アプリは転職用ポートフォリオ目的で一時的にデプロイして公開しています。現段階では以下の機能に限ってご利用いただけます。  
> 未実装または調整中の機能は停止しておりますので、ご了承ください。
> 今回の開発はフロントエンドとバックエンド両方携わりましたが、こちらはバックエンド中心に記述していきますので、フロントエンドのリポジトリはこちらからご確認ください。    
